                    ...
                    <div>
                        ...
                        <div class="my-3">
                            ...
                            <select name="user_id" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                ...
                                <option value="{{ $user->id }}" @selected(($user->id == $horario->user->id))>{{ $user->name.' '.$user->primerApellido.' '.$user->segundoApellido }}</option>
                                @endforeach
                            </select>
                            @error('name')
                                <p class="text-red-600">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-4 mb-3">
                            @foreach (App\Enums\DiasSemana::cases() as $dia)
                                <div class="mt-3">
                                    <input type="checkbox" {{ in_array($dia->value, $horario->dias ?? []) ? 'checked':'' }} id="{{ $dia->name }}" name="dias[]" value="{{ $dia->name  }}"  class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
                        ...
                        <div class="my-3">
                            <input value="{{ old('inicio', $horario->inicio) }}" placeholder="Ingrese la Hora de Entrada" name="inicio" type="time" class="border-gray-300 shadow-sm w-1/2 rounded-lg"  required>
                            @error('name')
                                <p class="text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="my-3">
                            <input value="{{ old('fin', $horario->fin) }}" placeholder="Ingrese la Hora de Salida" name="fin" type="time" class="border-gray-300 shadow-sm w-1/2 rounded-lg" required>
                            @error('name')
                                <p class="text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
