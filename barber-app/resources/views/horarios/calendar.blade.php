    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Horarios | Calendario') }}
            </h2>
            @can('ver horarios')
            <x-nav-link href="{{ route('horario.index') }}" class="bg-blue-500 text-white hover:text-white hover:underline px-2 py-2 pt-2 rounded-md inline-block flex items-center">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 w-full">
                @if($isAdmin)
                <form method="GET" action="{{ route('horario.calendar') }}">
                ...
                @else
                <span>Horario de {{ $user->nombre.' '.$user->primerApellido.' '.$user->segundoApellido }}</span>
                @endif
