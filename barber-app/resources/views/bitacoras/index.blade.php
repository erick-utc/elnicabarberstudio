    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 w-full grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {{ $roles->links() }} --}}
                 {{-- <x-nav-link href="{{ route('bitacora.accesos') }}" >Accesos </x-nav-link>
                 <x-nav-link href="{{ route('bitacora.movimientos') }}" >Movimientos</x-nav-link> --}}
                 <x-simple-card titulo="Accesos" descripcion="Registra los accesos al sistema" url="{{ route('bitacora.accesos') }}" />
                 <x-simple-card titulo="Movimientos" descripcion="Registra todos los movimientos realizados por los usuarios" url="{{ route('bitacora.movimientos') }}" />
            </div>
        </div>
    </div>
</x-app-layout>