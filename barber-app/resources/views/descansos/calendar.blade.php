    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Descansos | Calendario') }}
            </h2>
            @can('ver descansos')
            <x-nav-link href="{{ route('descanso.index') }}" class="bg-blue-500 text-white hover:text-white hover:underline px-2 py-2 pt-2 rounded-md inline-block flex items-center">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 w-full">
                @if($isAdmin)
                <form method="GET" action="{{ route('descanso.calendar') }}">
                ...
                @else
                <span>Descansos de {{ $user->nombre.' '.$user->primerApellido.' '.$user->segundoApellido }}</span>
                @endif
